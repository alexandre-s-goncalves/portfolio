...
describe('Spanish Translation Integrity System', () => {
  ...
  describe('CvModal Namespace', () => {
    ...
    test('should verify all structural cvModal dictionary keys exist in spanish', () => {
      expect(es).toHaveProperty(name);
      Object.values(keys).forEach(key => expect(es[name]).toHaveProperty(key));
    });

    test('should lock exact structural value parameters for spanish cvModal elements', () => {
      expect(es[name].brazilFormat).toBe('Formato Brasil');
  ...
  describe('NotFound Namespace', () => {
    ...
    test('should lock exact structural value parameters for notFound', () => {
      ...
      expect(es[name].description).toBe(
        'El enlace al que intentó acceder no se encontró o la ruta se movió no espaço-tempo. Utilice el botón de abajo para regresar de forma segura.',
      );
      expect(es[name].backButton).toBe('Volver al Inicio');
    });
  });
  ...
  describe('Projects Page Namespace', () => {
    ...
    test('should verify all structural projects dictionary keys exist', () => {
      expect(es).toHaveProperty(name);
      expect(es[name].status).toHaveProperty('completed');
      expect(es[name].status).toHaveProperty('progress');
      expect(es[name].items.portfolio).toHaveProperty('title');
      expect(es[name].items.portfolio).toHaveProperty('desc');
      expect(keys.title).toBeDefined();
    });
    ...
    test('should lock exact structural value parameters for projects page content', () => {
      ...
      expect(es[name].items.portfolio.title).toBe('Portafolio Personal');
    });
  });
  ...
  describe('Settings Namespace', () => {
    ...
    test('should lock exact structural value parameters for settings', () => {
      expect(es[name].title).toBe('Configuraciones');
      ...
      expect(es[name].themeDesc).toBe('Cambiar entre Modo Claro y Oscuro');
    });
  });
