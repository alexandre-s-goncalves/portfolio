...
export const BottomTabBar = () => {
  ...
  return (
    <div
      className={clsx(
        'fixed bottom-0 left-0 z-40 h-24 w-full bg-linear-to-t from-white via-white to-white/90 backdrop-blur-md md:hidden',
        'dark:from-slate-950 dark:via-slate-950 dark:to-slate-950/90',
      )}>
      <nav
        aria-label="Mobile Bottom Navigation"
        className={clsx(
          'fixed bottom-4 left-1/2 z-50 flex h-16 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 items-center justify-around rounded-2xl border px-2 shadow-xl transition-colors duration-300 select-none',
